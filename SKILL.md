...
description: |
  Campus Buddy v1.3 — 大学生全能成长平台，原创设计。非交互式参数驱动，AI 可直接调用。
  五大模块：学业（任务/课表/考试/作业/习惯/番茄/四象限/多学期）、学术（文献管理/引用格式转换/论文大纲/查重预检）、
  编程（即将上线）、求职（即将上线）、AI工具箱（即将上线）。
  课表支持图片识别导入（AI多模态），文献支持 GB/T 7714/APA/MLA 三格式互转。
  支持模块化 Web UI（零依赖，导航首页+独立模块界面，手机直连访问）。
  触发词：任务 | 清单 | 番茄 | 习惯 | 打卡 | 倒数日 | 四象限 | 课表 | 课表图片 | 导入课表 | 作业 | 考试 | 今日概览 | 学期 | 复习计划 | 文献 | 引用格式 | APA | MLA | 论文大纲 | 查重 | 相似度 | 日程 | Campus Buddy
---

# 🎓 Campus Buddy v1.3 — 大学生全能成长平台

> **不是日程工具，是成长操作系统。** 学业 + 学术 + 编程 + 求职 + AI工具箱，五大模块一个平台。
...
`student-assistant`（对外名称：Campus Buddy）— 大学生全能成长平台，非交互式参数驱动 + 移动端 Web UI

## 02 / Description — 何时加载

**Campus Buddy 是一个五大模块的成长平台，不是单一日程工具：**

- 📅 **学业模块**（已上线）：任务管理、课表、考试、作业、习惯、番茄、四象限、多学期、今日概览
- 📝 **学术模块**（已上线）：文献管理、引用格式转换、论文大纲、查重预检
- 💻 **编程模块**（即将上线）：算法刷题、代码片段、学习路径
- 🎯 **求职模块**（即将上线）：简历管理、面试题库、投递追踪
- 🤖 **AI工具箱**（即将上线）：AI写作辅助、知识问答、智能计划
...
- **番茄钟不要用 sleep** → v1.3 改为 `pomodoro log` 记录模式，避免 AI 调用超时
